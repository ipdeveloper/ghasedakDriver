...
public class MyLocationNewOverlay extends Overlay implements IMyLocationConsumer,
	...
	public void setDirectionArrow(final Bitmap personBitmap, final Bitmap directionArrowBitmap){
        ...
        float a =mPersonBitmap.getWidth() / 2.0f - 0.5f;
	...
	protected void drawMyLocation(final Canvas canvas, final MapView mapView, final Location lastFix) {
		...
		if (lastFix.hasBearing()) {
			...
			canvas.scale(1 / scaleX, 1 / scaleY, mMapCoordsTranslated.x, mMapCoordsTranslated.y);
			// Draw the bitmap
			canvas.drawBitmap(mPersonBitmap, mMapCoordsTranslated.x - (mPersonBitmap.getWidth() / 2.0f - 0.5f),
					mMapCoordsTranslated.y - (mPersonBitmap.getHeight() / 2.0f - 0.5f), mPaint);
			canvas.restore();
		}
	}
	...
	protected Rect getMyLocationDrawingBounds(double zoomLevel, Location lastFix, Rect reuse) {
		...
		if (lastFix.hasBearing()) {
			...
			reuse.offset((int) (-mPersonHotspot.x+1f ), (int) (-mPersonHotspot.y +1f));//+0.5f for x and y
		}
